...
export default class okex extends Exchange {
  ...
  async sub(symbol, type, range) {
    ...
    switch (type) {
      case 'depth': // 盘口
        this.send({ op: 'subscribe', args: [depth] });
        break;
      case 'kline': // K线
        this.send({ op: 'subscribe', args: [kline] });
        break;
      case 'trade': // 成交
        this.send({ op: 'subscribe', args: [trade] });
        break;
      default: {
        this.send({ op: 'subscribe', args: [depth, kline, trade] });
        break;
      }
    }
  }
  ...
  async unsub(type) {
    ...
    switch (type) {
      case 'depth': // 盘口
        this.send({ op: 'unsubscribe', args: [depth] });
        break;
      case 'kline': // K线
        this.send({ op: 'unsubscribe', args: [kline] });
        break;
      case 'trade': // 成交
        this.send({ op: 'unsubscribe', args: [trade] });
        break;
      default:
        this.send({ op: 'unsubscribe', args: [depth, kline, trade] });
        break;
    }
