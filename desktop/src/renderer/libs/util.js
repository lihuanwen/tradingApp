...
import { _timeStampToDate } from '@/libs/filters';

export function historyFormat(data) {
  let xAxisData = [];
  let seriesData = [];
  let list = selectionSort(Object.keys(data));
  list.forEach(key => {
    xAxisData.push(_timeStampToDate(key, 'MM-DD HH:mm:ss'));
...
export const getBreadCrumbList = (routeMetched, homeRoute) => {
  let res = routeMetched
    .filter(item => {
      return item.meta === null || !item.meta.hide;
    })
    .map(item => {
      let obj = {
        icon: (item.meta && item.meta.icon) || '',
...
/**
 * @param {*} access 用户权限数组，如 ['user', 'admin']
 * @param {*} route 路由列表
 */
